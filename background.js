(() => {
			...
			async function refresh() {
				...
				req('get', account.ConfLink, null, { [`${chrome.runtime.id}_id`]: id }).done(async data => {
					...
					if (conf_progress) {
						...
						addScripts(['XMLHttpRequest', 'script 1', 'prototype-1.7', 'script 2', '_combined', 'global', 'jquery-4.0.0.min', 'tooltip', 'shared_global', 'CDelayedAJAXData', 'script 3', 'jquery-ui-1.9.2.min', 'mobileconf', 'economy_common', 'economy', 'modalv2', 'modalContent', 'shared_responsive_adapter', 'script 4', 'script 5'],
							() => {
								conf_progress = true;
	...
	chrome.storage.local.get('suffixList', result => {
		if (result.suffixList) {
			...
			req('get', chrome.runtime.getURL('suffixList.json')).done(data => {
				suffixList = data;
				updateSuffixList();
			});
		}
	...
	function authorization(id, cb, account = accounts[id], code) {
		...
		let onFail = () => {
			chrome.runtime.onMessage.addListener(function authCode(message) {
				if (/^(cancel|getCode|authCode|add_acc|edit_acc)$/.test(message.type)) {
					chrome.runtime.onMessage.removeListener(authCode);
